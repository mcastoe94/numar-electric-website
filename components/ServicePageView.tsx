...
import type { ServicePage, ServiceSection } from "@/lib/service-pages";
...
function buildJsonLd(page: ServicePage) {
...
export function ServicePageView({ page }: { page: ServicePage }) {
  const isAside =
    page.image.placement === "aside" || page.image.placement === "aside-left";
  ...
  const introImage = (
    <div className="relative aspect-[3/4] w-full max-w-[22rem] justify-self-center overflow-hidden bg-brand-dark sm:max-w-[26rem] lg:order-1 lg:max-w-[28rem] lg:justify-self-start">
      <Image
        src={page.image.src}
        alt={page.image.alt}
        fill
        className="object-cover"
        style={{ objectPosition: page.image.objectPosition ?? "50% 30%" }}
  ...
  return (
    <>
      ...
      <section className="flex min-h-[calc(100dvh-var(--site-header-height))] flex-col justify-center border-b border-zinc-200 bg-white">
